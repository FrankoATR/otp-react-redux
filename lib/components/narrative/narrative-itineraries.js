...
import useWeatherRequests from '../map/use-weather-requests'
...
function getActiveActiveMode(hasWalk, hasBike) {
  if (hasWalk && !hasBike) return 'walking'
  if (hasBike && !hasWalk) return 'biking'
  // si tiene los dos (o ninguno) usa los dos
  return 'walking or biking'
}
...
class NarrativeItineraries extends Component {
  ...
  _renderItineraryRow = (itinerary, mini = false) => {
    ...
    /* ---------- weather logic ---------- */
    const hasWalk = itinerary.legs.some((l) => l.mode === 'WALK')
    const hasBike = itinerary.legs.some((l) => l.mode === 'BICYCLE')
    const activeMode = getActiveActiveMode(hasWalk, hasBike)

    const wFrom = weatherById.from  || {}
    const wTo   = weatherById.to    || {}

    let climateMessage = null

    if ((wFrom.loading || wTo.loading) && (hasWalk || hasBike)) {
      climateMessage = '🔄 Searching weather info...'
    } else if ((!wFrom.data || !wTo.data) && (hasWalk || hasBike)) {
      climateMessage = '⚠ No weather data found'
    } else if (raining && (hasWalk || hasBike)) {
      climateMessage = `⚠ It is raining. It's recommended to avoid ${activeMode}.`
    } else if (!raining && (hasWalk || hasBike)) {
      climateMessage = `🌤 The weather is good — ${
    ...
    return (
      <ListItem
        ...
        key={itinerary.index}
      >
        {climateMessage && itineraryIsExpanded && (
          <div
            style={{
              backgroundColor: raining ? '#fdecea' : '#e8f5e9',
              borderLeft: `4px solid ${raining ? '#f44336' : '#4caf50'}`,
              borderRadius: 4,
              color: raining ? '#b71c1c' : '#1b5e20',
              fontWeight: 'bold',
              marginBottom: 10,
              padding: 10
            }}
...
const mapStateToProps = (state) => {
  const weatherById = state.weather.byId
  const raining = 
    (weatherById.from?.data?.weathercode  ?? -1) >= 60 ||
    (weatherById.to  ?.data?.weathercode  ?? -1) >= 60
...
const ConnectedNarrative = connect(
  mapStateToProps,
  mapDispatchToProps
)(injectIntl(NarrativeItineraries))


export default function NarrativeWithWeather(props) {
  useWeatherRequests()
  return <ConnectedNarrative {...props} />
}
