...
class NarrativeItineraries extends Component {
  ...
  _renderItineraryRow = (itinerary, mini = false) => {
    ...
    const activeMode = getActiveActiveMode(hasWalk, hasBike) // <── NUEVO
    if (raining && (hasWalk || hasBike)) {
...
const mapStateToProps = (state) => {
  const weather = state.weather.byId
  const raining =
    weather?.from?.weathercode >= 60 || weather?.to?.weathercode >= 60
